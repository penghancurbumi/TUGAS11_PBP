...
app.get("/iphone", async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "handphone berhasil dipilih", handphone });
  } catch (error) {
...
app.get("/oppo", async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "handphone berhasil diambil", handphone });
  } catch (error) {
    res.status(500).json({ message: "Internal server error" });
  }
});
...
app.get("/poco", async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Pemain berhasil diambil", handphone });
  } catch (error) {
    res.status(500).json({ message: "Internal server error" });
  }
});
...
app.post("/jenis/:brand", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "seri handphone berhasil ditambahkan dengan merek " + brand });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal server error" });
  }
...
app.put("/jenis/:brand/:id", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Data brand handphone berhasil diupdate!" });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal server error" });
  }
...
app.delete("/jenis/:brand/:id", async (req, res) => {
  ...
  try {
    ...
    res.status(202).json({ message: "Data brand hanphone berhasil dihapus!" });
  } catch (error) {
    res.status(500).json({ message: "Internal server error" });
  }
});
